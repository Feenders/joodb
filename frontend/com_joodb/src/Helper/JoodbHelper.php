...
/**
 * General Halper for JooDB
 */
class JoodbHelper extends CMSHelper {
	 ...
	 *
	 * @access public
	 * @param JooDB-Object with fieldnames, Array with template parts, Object with Item-Data
	...
	public static function parseTemplate(&$joobase, &$parts, &$item,&$params = null) {
		...
		if (!empty($app->get('sef')) && !empty($falias) && !empty($item->{$falias})) {
			...
			$slug = $item->{$joobase->fid}.':'.OutputFilter::stringURLSafe($item->{$joobase->ftitle});
		}
		$itemlink = Route::_('index.php?option=com_joodb&view=article&id='.$slug.'&Itemid='.$Itemid,false);
				...
				if (!empty($part->function)) {
					switch ($part->function) {
						...
						case "path2item" :
							...
							break;
						case "path2editform" :
							$output .= Route::_('index.php?option=com_joodb&view=edit&joobase=' . $joobase->id . '&id=' . $item->{$joobase->fid}, false);
							break;
						case "nextbutton" :
						...
						case "thumb" :
							...
							break;
						case "path2image" :
							$output .= Uri::root(true) . (file_exists(JPATH_ROOT . $imgpart . ".jpg") ? $imgpart . ".jpg" : "/media/com_joodb/images/no_image.png");
							break;
						case "path2thumb" :
							$output .= Uri::root(true) . (file_exists(JPATH_ROOT . $imgpart . "-thumb.jpg") ? $imgpart . "-thumb.jpg" : "/media/com_joodb/images/no_image.png");
							break;
						case "checkbox" :
	...
	public static function replaceField(&$joobase, &$part, $field, $itemlink="",$id=0) {
		...
		if (!empty($itemlink) && $fieldname==$joobase->ftitle && $params->get('link_titles','0')!='0') {
			$itemlink = OutputFilter::ampReplace(htmlspecialchars($itemlink, ENT_COMPAT, 'UTF-8', false));
			$att = array();
			$att['title'] = Text::_("JDB_READ_MORE...");
			$att['class'] = "joodb_titlelink";
			$field= HtmlHelper::_('link', $itemlink,$field, $att);
		}
			...
			switch($function) {
				...
				case "longtext":
					if (($function=="varchar" || $function=="tinytext") && $params->get('link_urls','0')!='0') {
						...
						if (preg_match('/^[^@]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$/', $field)) {
							$field= HtmlHelper::_('email.cloak', $field);
						} else if (strtolower(substr($field,0,4))=="www." || preg_match('#^http(s)?://#',$field)) {
							$ltext = preg_replace( "#^[^:/.]*[:/]+#i", "", $field);
							$link = OutputFilter::ampReplace(htmlspecialchars($field, ENT_COMPAT, 'UTF-8', false));
							if (strtolower(substr($field,0,4))=="www.") {
								$link = "//".$link;
							}
							$field= HtmlHelper::_('link', $link,$ltext, array("target"=>"_blank"));
						}
					}
					...
					break;
				case "date":
					$field= HtmlHelper::_('date', $field, Text::_('DATE_FORMAT_LC3'));
					break;
				case "datetime":
					$field= HtmlHelper::_('date', $field, Text::_('DATE_FORMAT_LC2'));
					break;
				case "timestamp":
					$field= HtmlHelper::_('date', $field, Text::_('DATE_FORMAT_LC2'));
					break;
				...
				case "float" :
					if (isset($part->parameter[0])) {
						$field= number_format($field, (int)$part->parameter[0], Text::_('DECIMALS_SEPARATOR'), Text::_('THOUSANDS_SEPARATOR'));
					}
					break;
				case "decimal" :
				...
				case "longblob" :
					...
					$field = (!empty($field) && !empty($id)) ? Uri::root().'index.php?option=com_joodb&task=getFileFromBlob&joobase='.$joobase->id.'&id='.$id.'&field='.$fieldname : "";
					break;
			}
		}
		return $field;
	...
	public static function getPrintPopup(&$item, &$joobase, $attribs = array())
	{
		...
		if ( $params->get( 'show_icons', '0' )==1) {
			$text = HtmlHelper::image('media/com_joodb/images/print.png', Text::_("JDB_PRINT") );
		} else {
			$text = '<span class="jicon jicon-print"></span>&nbsp;'. Text::_("JDB_PRINT");
		}

		$attribs['title']	= Text::_("JDB_PRINT");
		$attribs['onclick'] = "window.open(this.href,'win2','".$status."'); return false;";
		$attribs['rel']     = 'nofollow';

		return HtmlHelper::link(Route::_($url), $text, $attribs);
	}
	...
	public static function getNotepadButton(&$item, &$joobase)
	{
		...
		if ($input->get('tmpl')!='component') {
			...
			$urltext = ($input->get('layout')=="notepad") ?  Text::_("JDB_REMOVE_FROM_NOTEPAD") : Text::_("JDB_ADD_TO_NOTEPAD");
			// checks template image directory for image, if non found default are loaded
			if ( $params->get( 'show_icons', '0')==1) {
				$icon = ($input->get('layout')=="notepad") ? "remove.png" : "add.png";
				$text = HtmlHelper::image('media/com_joodb/images/'.$icon,$urltext );
			} else {
	...
	public static function getEditButton(&$item, &$joobase,&$part)
	{
		...
		if ($app->input->get('tmpl')!='component') {
			...
			$url  = Route::_("index.php?option=com_joodb&view=".$view."&joobase=".$joobase->id."&id=".$item->{$joobase->fid}."&Itemid=".$app->input->getInt('Itemid'));
			...
			if ($params->get( 'show_icons', '0' )==1) {
				$icon = "edit.png";
				$text = HtmlHelper::image('media/com_joodb/images/'.$icon,Text::_("JDB_EDIT_DATABASE_ENTRY"));
			} else {
				$text = '<span class="jicon jicon-edit"></span>&nbsp;'.Text::_("JDB_EDIT_DATABASE_ENTRY");
			}
			$attribs= array('title'=> Text::_("JDB_EDIT_DATABASE_ENTRY"));
			return HtmlHelper::link(Route::_($url), $text,$attribs);
		}
	}
	...
	public static function getDeleteButton(&$item, &$joobase) {
		...
		if ($params->get('show_icons','0')==1) {
			...
			$text = '<span class="jicon jicon-trash"></span>&nbsp;'.Text::_("JDB_DELETE");
		}
		$attribs= array('title'=> Text::_("JDB_DELETE_DATABASE_ENTRY"),'onclick'=>'return confirm(\''.Text::_("JDB_REALLY_DELETE").'\');');
		return HtmlHelper::link(Route::_($url), $text,$attribs);
	}
	...
	public static function getBackbutton($joobase) {
		$app = Factory::getApplication();
		$url  = Route::_("index.php?option=com_joodb&view=catalog&joobase=".$joobase->id."&Itemid=".$app->input->getInt('Itemid'));
		$params	= ComponentHelper::getParams('com_joodb');
		if ($params->get('show_icons','0')==1) {
			$text = HtmlHelper::image('media/com_joodb/images/back.png', Text::_("JDB_BACK") );
		} else {
			$text = '<span class="jicon jicon-left-open"></span>&nbsp;'.Text::_("JDB_BACK");
		}
		return HtmlHelper::link($url, $text,array('title'=>  Text::_("JDB_BACK"),'class'=>'backbutton'));
	}
	...
	public static function getReadmore($url) {
		$params	= ComponentHelper::getParams('com_joodb');
		if ($params->get('show_icons','0')==1) {
			$text = HtmlHelper::image('media/com_joodb/images/next.png',Text::_("JDB_READ_MORE..."));
		} else {
			$text = Text::_("JDB_READ_MORE...").'&nbsp;<span class="jicon jicon-right-open"></span>';
		}
		return  HtmlHelper::link($url , $text,array('title'=>  Text::_("JDB_READ_MORE..."),'class'=>'readonbutton'));
	}
	...
	public static function getNavigationButton(&$joobase,$side="next") {
		...
		$url = Route::_('index.php?option=com_joodb&view=article&joobase='.$joobase->id.'&id='.$slug."&position=".$item->jb_pos."&total=".$item->jb_total,false);
		if ($side=="next") {
			$btext = Text::_("JDB_NEXT_ENTRY").'&nbsp;<span class="jicon jicon-right-open"></span>';
			$image = "next.png";
		} else {
			$btext = '<span class="jicon jicon-left-open"></span>&nbsp;'.Text::_("JDB_PREVIOUS_ENTRY");
			$image = "back.png";
		}
		$params	= ComponentHelper::getParams('com_joodb');
		if ($params->get('show_icons','0')==1) {
			$text = HtmlHelper::image('media/com_joodb/images/'.$image, strip_tags($btext) );
		} else {
			$text = $btext;
		}
		return HtmlHelper::link($url,$text,array('title'=>  strip_tags($btext),'class'=>$side.'button'));
	}
	...
	public static function getSearchbox($search="",$parameter = array())
	{
		...
		if (!empty($parameter[0])) {
			$fields = @preg_split("/,/",(string)$parameter[0]);
			$searchform .= "<select class='form-select' name='searchfield'><option value=''>".Text::_("JDB_ALL_FIELDS")."</option>" ;
		...
		$searchform .=  "<button class='btn btn-primary btn-search' type='submit' ><span class='jicon-search'></span>&nbsp;".$stext."</button>"
		                ."<button class='btn btn-dark btn-reset' type='button' onclick='submitSearch(\"reset\");void(0);' ><span class='jicon-cancel'></span>&nbsp;".Text::_("JDB_RESET...")."</button></div>";
		return $searchform;
	}
	...
	public static function getAlphabox($alphachar, &$joobase)
	{
		...
		$alphabox .=  "<li><a class='reset' href='Javascript:submitSearch(\"reset\");void(0);'>&raquo;".Text::_("JDB_ALL")."</a></li></ul></div>";
		return $alphabox;
	}
	...
	public static function getOrderlink(&$parameter,&$joobase)
	{
		...
		if ($app->getUserStateFromRequest('com_joodb.orderby', 'orderby', $params->get('orderby','fid'), 'string')==$parameter[0]) {
			$ordering = (strtolower($app->input->getCmd('ordering','asc')) == "asc") ? "DESC" : "ASC";
			$orderclass = (strtolower($app->input->getCmd('ordering','asc')) == "asc") ? "-down" : "-up";
		}
	...
	public static function getCaptcha(){
		$captcha ="<div class='joocaptcha' style='margin: 5px 0;' >"
		          ."<img src='".Uri::root(false)."index.php?option=com_joodb&task=captcha&".microtime(true)."' alt='captcha' style='width:200px; height:50px; margin: 5px 0; border: 1px solid black;'  />"
	...
	public static function _findItem(&$joobase,$params="")
	{
		...
		return Route::_("index.php?option=com_joodb&view=catalog&joobase=".$joobase->id."&Itemid=".$Itemid.$params,false);
	}
	...
	public static function checkAuthorization(&$joobase, $section="accessd",&$item=null) {
		$jparams = new Registry($joobase->params);
		...
		if ($section=="accesse") {
			...
			if (!$has_access) {
				...
				$uri = Uri::getInstance();
				$return	= $uri->toString();
				$url  = Route::_('index.php?option=com_users&view=login&return=');
				$url .= base64_encode($return);
				$app = Factory::getApplication();
				$app->enqueueMessage(Text::_("JDB_PLEASE_LOGIN"),"warning");
				$app->redirect($url, 402 );
				die();
			}
		}
		return $has_access;
	...
	public static function prepareDocument() {
		...
		$app = Factory::getApplication();
		HTMLHelper::_('stylesheet', 'com_joodb/icons.css', array('version' => 'auto', 'relative' => true));
		if (file_exists(JPATH_BASE."/templates/".$app->getTemplate()."/css/joodb.css")) {
			HTMLHelper::_('stylesheet', "templates/".$app->getTemplate()."/css/joodb.css");
		} else {
			HTMLHelper::_('stylesheet', 'com_joodb/joodb.css', array('version' => 'auto', 'relative' => true));
		}

		HTMLHelper::_('script', 'com_joodb/featherlight.min.js', array('version' => 'auto', 'relative' => true));
		HTMLHelper::_('stylesheet', 'com_joodb/featherlight.min.css', array('version' => 'auto', 'relative' => true));
	}
	 ...
	 * @return \stdClass
	 */
	public static function getJoodbPart()
	{
		$p = new \stdClass();
		$p->function = false; //the joodb function
		$p->parameter = array(); //an array of parameters
		$p->text = ""; // the text to the next command
		$p->pval = null; // Parameter Values
		return $p;
	}
