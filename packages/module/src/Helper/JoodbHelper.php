...
namespace Feenders\Module\Joodb\Site\Helper;
...
class JoodbHelper {
	...
	public static function getRoute(&$jb,&$item)
	{
		...
		if (!empty($app->get('sef')) && !empty($falias) && !empty($item->{$falias})) {
			...
			$slug = $item->{$jb->fid}.':'.OutputFilter::stringURLSafe($item->{$jb->ftitle});
		}
		return Route::_('index.php?option=com_joodb&view=article&joobase='.$jb->id.'&id='.$slug.$catalog,false);
	}
