 ...
 * JooDB item Table
 */
class JoodbTable extends Table {
	...
	public function bind($src,$ignore=array()) {
		...
		if ($p->get('extdb_server')!="") {
			...
			try {
				...
				$this->_tbldb = $dbf->getDriver('mysqli',$options);
			} catch (Exception $e) {
	...
	public function check()
	{
		...
		if(empty($this->name)) {
			$this->setError(Text::_('Database must have a name'));
			return false;
		}
		if(empty($this->table)) {
			$this->setError(Text::_("JDB_PLEASE_CHOOSE_TABLE"));
			return false;
		}
		if(empty($this->fid)) {
			$this->setError(Text::_("JDB_ERROR_DEFINE_FIELDS"));
			return false;
		}
	 ...
	 */
	public function load($keys=null,$reset=false)
	{
	...
	public function getDefaultTemplate($tname) {
		...
		if (file_exists($tfile)) {
			...
			if ($tname=="listview") {
				$header = Text::_("JDB_TITLE").'<span style="float:right">'.Text::_("JDB_ORDER_BY")
				...
				if (!empty($this->fdate)) {
					$loop .= "<span class='small'>".Text::_("JDB_DATE").": {joodb ".$this->fdate."}</span>";
				}
				...
				$content =  (!empty($this->fdate)) ? Text::_("JDB_DATE").": {joodb field|".$this->fdate."}" : "";
