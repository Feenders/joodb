 ...
 * Admin helper class ...
 */
class JoodbAdminHelper extends CMSHelper {
	...
	/**
	 * returns an optionlist with all availiable functions
	 *
	 * @param $editorid
	 * @param $fieldlist
	 * @param $context
	...
	public static function printTemplateFooter($editorid,$fieldlist,$context) {
		echo "<table class='mt-3 paramlist admintable' style='width: 100%;'><tr><td class='paramlist_key'>".Text::_("JDB_INSERT_FIELD")."</td><td class='paramlist_value'>";
		...
		echo '</select>';
		echo "</td><td class='paramlist_key'>".Text::_("JDB_INSERT_FUNCTION")."</td><td class='paramlist_value'>";
	...
	public static function selectFieldTypes($type, $fields) {
		...
		foreach ($fields as $fcell) {
			if ($type=="primary") {
				...
				if (str_contains($fcell->Type, "text")) $fselect[] = $fcell->Field;
			} else if ($type=="shorttext") {
				if (str_contains($fcell->Type,"varchar")!==false) $fselect[] = $fcell->Field;
				if (str_contains($fcell->Type,"text")!==false) $fselect[] = $fcell->Field;
			} else if ($type=="date") {
				if (str_contains($fcell->Type, "date")) $fselect[] = $fcell->Field;
				if (str_contains($fcell->Type,"timestamp")!==false) $fselect[] = $fcell->Field;
			} else if ($type=="number") {
				if (str_contains($fcell->Type,"int")!==false) $fselect[] = $fcell->Field;
			}
		}
		return $fselect;
	}
	...
	public static function prepareDocument() {
		...
		HtmlHelper::_('stylesheet', 'com_joodb/joodb-admin.css', array('version' => 'auto', 'relative' => true));

		HtmlHelper::_('jquery.framework');

		HtmlHelper::_('script', 'com_joodb/joodb-admin.js', array('version' => 'auto', 'relative' => true));
