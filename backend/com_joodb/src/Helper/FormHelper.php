...
namespace Feenders\Component\Joodb\Administrator\Helper;
 ...
 */
class FormHelper extends CMSHelper {
			...
			switch ($typearr[0]) {
				...
				case 'real' :
					if ($name==$joobase->getSubdata('fuser')) {
						if ($app->getIdentity()->authorise('core.manage', 'com_users'))
						{
				...
				case 'tinyint' :
					if ((!empty($joobase->fstate) && $joobase->fstate==$name) || ($param=="yesno")) {
						$result .=  '<select class="form-select input-small" name="'.$name.'" id="'.$fid.'"><option value="0">'.Text::_("JDB_NO").'</option><option value="1" ';
						if (!empty($item->{$name})) $result .= 'selected="selected"';
						$result .= '>'.Text::_("JDB_YES").'</option></select>';
					} else {
				...
				case 'timestamp' :
					...
					$result .= HtmlHelper::_('calendar', $item->{$name} , $name, $fid, '%Y-%m-%d %H:%M:%S', array('class'=>'form-control input-medium '.$required, 'size'=>'25',  'maxlength'=>'19'));
					break;
				case 'date' :
					$item->{$name} = preg_replace("/[^0-9\-]/","",$item->{$name});
					$result .= HtmlHelper::_('calendar', $item->{$name} , $name, $fid, '%Y-%m-%d', array('class'=>'form-control input-small '.$required, 'size'=>'25',  'maxlength'=>'10'));
					break;
				case 'year' :
				...
				case 'longtext' :
					...
					if ($param=="textarea") {
						...
						if (empty(self::$editor)) {
							self::$editor = new Editor($app->get('editor'));
						}
						$result .= self::$editor->display($name, stripslashes($item->{$name}), '100%', '280', '60', '10',false,"joodb_".$fid, array('class'=>'form-control'));
					}
					$result .= '</div>';
					break;
				...
				case 'set' :
					...
					if (count($typevals)<=3 || $param=="check") {
						...
						$result .= '<select class="form-select input-xxlarge js-choice" '.$required.' type="text" style="width: 100%;" multiple="multiple" name="'.$name.'[]" id="'.$fid.'" >';
						foreach ($typevals as $value) {
							$value = str_replace("''","'",$value);
				...
				case 'longblob' :
					...
					$result .=  '&nbsp;<label  class="form-check-input" ><input  class="form-check-input" type="checkbox" name="'.$name.'_del" value=1 />&nbsp;'.Text::_("JDB_EMPTY_FIELD").'</label>';
					if (!empty($item->{$name})) {
						$mime = JoodbAdminHelper::getMimeType($item->{$name});
						$fileurl = Uri::root().'index.php?option=com_joodb&task=getFileFromBlob&joobase='.$joobase->id.'&id='.$item->{$joobase->fid}.'&field='.$name;
						...
						if (substr($mime, 0,5)=="image") {
							...
							$result .= ' &raquo;<a href="'.$fileurl.'" target="_blank">'.Text::_("JDB_DOWNLOAD").'</a>&laquo;';
						}
						$result .= '</div>';
					}
					break;
	...
	static function saveData(&$jb,&$item,$copy=false) {
		...
		foreach ($fields as $fname=>$fcell) {
			...
			if (isset($_POST[$fne]) || isset($_FILES[$fne])) {
				...
				if ($fcell->Null=="NO" && ($item->{$fname}===null)) {
					$errors++;
					$app->enqueueMessage( Text::_( 'JLIB_FORM_FIELD_INVALID' ).$fname,'error');
				}
			} else {
		...
		if (!empty($falias)) {
			if (empty($item->{$falias}) || empty($item->{$fid})) {
				$item->{$falias} = OutputFilter::stringURLSafe( $item->{$jb->ftitle});
			} else {
				$item->{$falias} = OutputFilter::stringURLSafe( $item->{$falias});
			}
		...
		try {
			...
			$app->enqueueMessage(Text::_("JDB_ERROR") . " : " . $e->getMessage(), 'error');
			return false;
		}

		$app->enqueueMessage( Text::_("JDB_ITEM_SAVED"));
