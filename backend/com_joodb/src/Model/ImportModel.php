...
class ImportModel extends BaseDatabaseModel {
	...
	private function generateTable(&$ws) {
		...
		foreach ($cellIterator as $i => $cell)
			if (!is_null($cell)) {
				...
				if ($this->columns[$i]->type=="s" || Date::isDateTime($dcell)) {
	...
	public function importSheet($file) {
		...
		if (!File::upload($file['tmp_name'], $this->file)) {
		...
		if ($this->format!=false) {
			...
			if ($this->generateTable($ws)) $this->message = Text::_("JDB_TABLE_CREATED");
			$this->startRow = ($this->has_column_names==0) ? 1 : 2;
		} else {
			$this->setError(Text::_("JDB_ERROR_IMPORTING_TABLE")." : ".Text::_("JDB_UPLOAD_VALID_EXCEL_FILE"));
			return false;
		}
		return true;
	}
					...
					if (!is_null($cell) && isset($cval["title"])) {
						if ($cval['type']=="text") {
							...
							if ($cval['type']=="float" || $cval['type']=="int(8)") {
								...
								$value = $cell->getValue();
								if (!empty($value)) {
									$ts = Date::excelToTimestamp($value);
